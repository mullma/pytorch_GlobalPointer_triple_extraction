...
class GlobalPointerRe(nn.Module):
    def __init__(self, args):
        ...
        self.bilstm = nn.LSTM(self.hidden_size, self.lstm_hiden, 1, bidirectional=True, batch_first=True,
                              dropout=0.1)
        self.entity_output = EfficientGlobalPointer(self.hidden_size, heads=2, head_size=64)
        self.head_output = EfficientGlobalPointer(self.hidden_size, heads=args.num_tags, head_size=64, RoPE=False, tril_mask=False)
        self.tail_output = EfficientGlobalPointer(self.hidden_size, heads=args.num_tags, head_size=64, RoPE=False, tril_mask=False)
